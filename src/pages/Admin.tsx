...
import type { UsuarioPapel } from '../types/api';

type Tab = 'dashboard' | 'usuarios' | 'grupos' | 'arquivamento' | 'erros';
type AdminUser = Awaited<ReturnType<typeof api.adminUsuarios>>[number];
const nav = [{ id: 'dashboard' as const, label: 'Dashboard', icon: LayoutDashboard }, { id: 'usuarios' as const, label: 'Usuários', icon: Users }, { id: 'grupos' as const, label: 'Grupos', icon: FolderKanban }, { id: 'arquivamento' as const, label: 'Arquivamento', icon: FileArchive }, { id: 'erros' as const, label: 'Erros do sistema', icon: AlertTriangle }];
...
export function Admin() {
  ...
  return <div className="nfse-admin">
    ...
    <div className="adm-page">
      ...
      <main className="adm-content"><div className="adm-title"><div><h1>{tab === 'dashboard' ? 'Dashboard' : tab === 'usuarios' ? 'Usuários' : tab === 'grupos' ? 'Grupos' : tab === 'arquivamento' ? 'Arquivamento' : 'Erros do sistema'}</h1><p>{tab === 'dashboard' ? 'Visão geral da operação NFS-e' : tab === 'usuarios' ? 'Gerencie contas, grupos e permissões' : tab === 'grupos' ? 'Crie e organize os grupos do portal' : tab === 'arquivamento' ? 'Mantenha no portal somente o acervo operacional de 2026' : 'Acompanhe falhas dos processamentos'}</p></div><span>Home / {tab === 'dashboard' ? 'Dashboard' : tab === 'usuarios' ? 'Usuários' : tab === 'grupos' ? 'Grupos' : tab === 'arquivamento' ? 'Arquivamento' : 'Erros'}</span></div>
        ...
        {tab === 'arquivamento' ? <ArchivePanel /> : null}
...
function ArchivePanel() {
  ...
  const qc = useQueryClient();
  const status = useQuery({ queryKey: ['admin-arquivamento', 2026], queryFn: () => api.adminArquivamento(2026), refetchInterval: query => ['executando', 'agendado'].includes(query.state.data?.execucao.status || '') ? 3000 : 30000 });
  const executar = useMutation({ mutationFn: () => api.adminExecutarArquivamento(2026, confirmacao), onSuccess: () => { setConfirmacao(''); void qc.invalidateQueries({ queryKey: ['admin-arquivamento'] }); toast.success('Arquivamento agendado'); }, onError: (error: Error) => toast.error(error.message) });
  const data = status.data;
  const emExecucao = ['executando', 'agendado'].includes(data?.execucao.status || '');
  return <div className="adm-archive-grid"><section className="adm-card"><CardHeader title="Política do acervo" icon={<FileArchive />} action={<button onClick={() => status.refetch()}><RefreshCcw className={status.isFetching ? 'spin' : ''} /> Atualizar</button>} /><div className="adm-archive-body"><p>As notas de <strong>2026</strong> permanecem no portal. Notas de anos anteriores são empacotadas por empresa, enviadas ao armazenamento de arquivos e só então marcadas como arquivadas.</p><div className="adm-archive-numbers"><span><strong>{(data?.notas_elegiveis || 0).toLocaleString('pt-BR')}</strong> elegíveis</span><span><strong>{(data?.notas_arquivadas || 0).toLocaleString('pt-BR')}</strong> já arquivadas</span><span><strong>{data?.backups.length || 0}</strong> backups recentes</span></div><label>Confirmação<input value={confirmacao} onChange={e => setConfirmacao(e.target.value)} placeholder="Digite ARQUIVAR 2026" /></label><button className="adm-archive-run" disabled={confirmacao.trim().toUpperCase() !== 'ARQUIVAR 2026' || executar.isPending || emExecucao} onClick={() => executar.mutate()}>{emExecucao ? 'Arquivamento em execução…' : 'Criar backups e arquivar'}</button>{data?.execucao.erro ? <p className="adm-archive-error">{data.execucao.erro}</p> : null}<small>Esta ação nunca exclui as notas do banco. A confirmação evita execuções acidentais.</small></div></section><section className="adm-card"><CardHeader title="Histórico de backups" icon={<FileText />} /><div className="adm-archive-history">{(data?.backups || []).map(item => <article key={item.id}><div><strong>{item.filename}</strong><small>Empresa #{item.empresa_id || '-'} · {formatDateTime(item.created_at)}</small></div><span>{item.tamanho_bytes ? `${(item.tamanho_bytes / 1024 / 1024).toFixed(1)} MB` : '-'}</span></article>)}{!data?.backups.length ? <div className="adm-empty">Nenhum backup criado ainda.</div> : null}</div></section></div>;
}
