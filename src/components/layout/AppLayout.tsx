...
function AppLayoutContent() {
  ...
  return (
    <OperatorGate>
      <div className="portal-app min-h-screen w-full min-w-0 overflow-x-hidden">
        <CertificadoExpiryTicker />
        <div aria-hidden="true" className={classNames('fixed left-0 top-0 z-[100] h-0.5 bg-primary shadow-[0_0_10px_rgb(var(--primary))] transition-all duration-300', navigation.state === 'idle' ? 'w-0 opacity-0' : 'w-2/3 opacity-100')} />
        ...
        <div className={classNames('min-w-0 overflow-x-hidden transition-[padding] duration-200', focusMode ? 'lg:pl-0' : sidebarCollapsed ? 'lg:pl-20' : 'lg:pl-72')}>
          ...
          <main className="w-full min-w-0 overflow-x-hidden px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
            <Suspense
              fallback={
                <div className="flex items-center justify-center gap-2 py-24 text-text-secondary" role="status">
